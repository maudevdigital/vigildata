...
Todas las pruebas fueron diseñadas siguiendo el patrón Arrange-Act-Assert, separando claramente la preparación de los datos (Arrange), la ejecución de la función (Act) y la validación de los resultados (Assert).
...
Se verificó además la cobertura de ramas (Branch Coverage) cumpliendo con el mínimo del 70% exigido, validando las decisiones lógicas de if/else en los endpoints.

### Cumplimiento del requisito de cobertura

| Requisito | Resultado |
|----------|-----------|
| Cobertura minima solicitada (líneas) | 80% |
| Cobertura minima solicitada (ramas) | 70% |
| Cobertura obtenida (líneas) | 82.47% |
| Cobertura obtenida (ramas) | >70% |
| Estado | Cumplido |
...
También tuvimos cuidado de no caer en el anti-patrón del exceso de mocking, asegurándonos de simular solo las dependencias externas (como la base de datos) y probando la lógica real de negocio con objetos concretos cuando era posible.
...
Gracias a la implementación de estas herramientas, nuestras pruebas cumplen con el Principio FIRST, ya que son rápidas de ejecutar, están aisladas (gracias al mocking), y son auto-validables.

## Conclusion

El taller fue aplicado sobre funcionalidades reales de VigilData. Se implementaron 12 pruebas unitarias, se uso mocking para aislar dependencias externas y se obtuvo una cobertura total de líneas de 82.47%, superando el minimo solicitado de 80%, además de cumplir con más del 70% de cobertura de ramas requerida
