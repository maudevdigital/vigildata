...
def listar_incidentes(
    ...
    query = _aplicar_filtros(
        db.query(Incidente),
        region=region, comuna=comuna, nivel_riesgo=nivel_riesgo,
...
def resumen_incidentes(
    ...
    total = _aplicar_filtros(db.query(Incidente), **filtros).count()
    ...
    return IncidenteResumenResponse(
        total=total,
        por_comuna=[ConteoItem(etiqueta=r.etiqueta, total=r.total) for r in por_comuna_rows],
        por_tipo=[ConteoItem(etiqueta=r.etiqueta, total=r.total) for r in por_tipo_rows],
    )
