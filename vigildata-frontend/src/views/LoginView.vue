...
async function handleGoogleLogin() {
  ...
  try {
    ...
    error.value = e.response?.data?.detail || e.message || 'No se pudo iniciar sesión con Google'
  } finally {
    cargando.value = false
  }
}
...
<template>
  <div class="flex justify-center mt-16">
    <form @submit.prevent="handleLogin" class="bg-white p-8 rounded-lg shadow-md w-96">
      ...
      <button type="submit" :disabled="cargando" class="w-full bg-blue-600 text-white py-2 rounded hover:bg-blue-700 disabled:opacity-60 touch-target">
